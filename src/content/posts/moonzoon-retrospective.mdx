...
Many ideas from MoonZoon carried forward into [Boon](/#boon). The reactive signals architecture evolved into Boon's dataflow model. The frustration with HTML/CSS abstraction led to new rendering approaches. And the WASM size battles? In Boon, that burden shifts from every app developer to a platform maintainer.
