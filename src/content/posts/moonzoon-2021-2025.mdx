...
[MoonZoon: Foundations (2021)](/blog/moonzoon-foundations-2021) summarized the early dev.to articles about [MoonZoon's](/#moonzoon) core concepts—signals over hooks, TEA vs components, fullstack architecture with virtual actors, and the WASM size optimization battles.
...
<Note>
The `#[s_var]` macro was an early API experiment. The modern MoonZoon routing uses a cleaner derive-based approach:
...
ROUTER.go(Route::Active);

// Or use type-safe links
Link::new().label("Active").to(Route::Active)
```
</Note>
...
<Note>
SharedArrayBuffer enables true shared memory between threads, but comes with significant constraints:

- **Security headers required:** The server must send `Cross-Origin-Opener-Policy: same-origin` and `Cross-Origin-Embedder-Policy: require-corp`
- **HTTPS required:** These headers are only respected over HTTPS (or localhost)
- **Nightly Rust required:** WASM atomics need `-Z build-std` and the nightly toolchain
- **Special RUSTFLAGS:** `-C target-feature=+atomics,+bulk-memory,+mutable-globals`
- **LTO breaks things:** Link-time optimization must be disabled when building with multithreading support
- **Different wasm-bindgen output:** The output format changes from `"web"` to `"no-modules"` for multithreading

MoonZoon's approach favored serialization for simplicity—the complexity of SharedArrayBuffer rarely justified the performance gains for typical web app workloads.
</Note>
...
<Note>
This limitation was one motivation for experimenting with [Boon](/#boon), a language I'm developing. Rust's type system can't fully prevent these threading footguns in WASM—the browser's constraints don't map cleanly to Rust's `Send`/`Sync` model.
</Note>
...
<Note>
Both Tauri and wgpu integrations were exploratory—proving that MoonZoon's architecture could extend beyond the browser. The wgpu work in particular informed the development of [NovyWave](/#novywave), a separate GPU-focused project.
</Note>

### Tauri: Native Desktop Applications

March 2024 brought [Tauri](https://tauri.app/) v2 integration. Tauri wraps web applications in native windows, with Rust on the backend instead of Node.js (like Electron). A MoonZoon app could become a desktop app with minimal changes.
...
May 2024 added GPU rendering through [wgpu](https://wgpu.rs/), the Rust WebGPU implementation:
...
<Note>
The `color!` macro uses Servo's CSS color parser under the hood, so it accepts any CSS color format—not just oklch:

```rust
color!("#ff6600")                          // hex
color!("rgb(255, 102, 0)")                 // rgb
color!("hsl(24, 100%, 50%)")               // hsl
color!("oklch(0.7 0.15 50)")               // oklch
color!("oklab(0.7 0.1 0.1)")               // oklab
color!("color(display-p3 1 0.5 0)")        // wide gamut
color!("oklch(0.6 0.182 350.53 / .7)")     // with alpha
```

The `oklch()` builder is a convenience helper for runtime color construction. Both approaches produce typed color values validated at compile time.
</Note>
...
March 2025 removed the embedded Boon interpreter from MoonZoon (-5,343 lines). The experiment didn't end—it graduated. [Boon](/blog/the-boon-of-dataflow) evolved into a standalone language with its own vision: dataflow programming that makes time manipulation natural. The ideas that started as a MoonZoon scripting layer became something bigger.
...
- **Color is hard.** Pick a perceptually uniform color space (oklch today) and build your palette tools around it. sRGB HSL will cause problems. Consider using Servo's `cssparser-color` crate—it handles all CSS color formats and saves you from writing your own parser.
...
![Browser logos: Chrome, Firefox, Safari, Edge](/images/browsers.png)
...
- **Test on multiple browsers.** They all have different quirks, bugs, and implementations. Notes from debugging on Firefox:
   ...
   - **Scale animation jitter:** Firefox needs `translateZ(0.001px)` to force GPU layer composition and smooth scale animations
   - **WebGPU/wgpu window sizing:** Firefox requires both `with_max_inner_size()` AND `with_inner_size()` to be set
   - **CSS Font Face rules:** Firefox throws errors when setting properties on `CSSFontFaceRule`—needs graceful error handling
   - **ResizeObserver differences:** Firefox returns `borderBoxSize` as an object, Chrome returns an array
   - **Async timing:** Firefox has an event loop bug that causes race conditions. I tried to work around it by adding a synchronization Future to the browser executor, but the Rust executor maintainers rejected it due to performance concerns.
...
*This post covers MoonZoon development from July 2021 through 2025. For the foundational concepts and the dev.to article series summary, see [MoonZoon: Foundations (2021)](/blog/moonzoon-foundations-2021). For where these ideas went next, see [The Boon of Dataflow](/blog/the-boon-of-dataflow).*
