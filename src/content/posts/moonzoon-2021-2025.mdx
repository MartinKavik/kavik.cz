...
The [previous retrospective](/blog/moonzoon-retrospective) covered [MoonZoon's](/#moonzoon) foundational concepts—signals over hooks, TEA vs components, and the lessons from the dev.to article series. That post ended in July 2021.
...
<Note>
This limitation was one motivation for experimenting with Boon, a custom scripting language. Rust's type system can't fully prevent these threading footguns in WASM—the browser's constraints don't map cleanly to Rust's `Send`/`Sync` model.
</Note>
...
*This post covers MoonZoon development from July 2021 through 2025. For the foundational concepts and the dev.to article series summary, see [MoonZoon: Retrospective](/blog/moonzoon-retrospective). For where these ideas went next, see [The Boon of Dataflow](/blog/the-boon-of-dataflow).*
